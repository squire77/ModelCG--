package scaffold.platform.graphics.graph;

import scaffold.platform.graphics.draggable.DraggableCanvas;
...
public class GraphCanvas extends DraggableCanvas {    
    ...
    public void writeGraph(String graphFileNameAbsolutePath) {
        ...
        try {
            FileUtility.writeFile(graphFileNameAbsolutePath, strBuf.toString());
        } catch (IOException ex) {
            ErrorDialog.error("Unable to write graph: " + graphFileNameAbsolutePath);
        }
    }
    ...
    public void readGraph(String graphFileNameAbsolutePath) {
        ...
        String graphDesc;
        try {
            graphDesc = FileUtility.readFile(graphFileNameAbsolutePath);
        } catch (IOException ex) {
            ErrorDialog.error("Unable to open graph: " + graphFileNameAbsolutePath);
            return;
        }
        ...
        if (graphData.length < 2) {
            ErrorDialog.error("Invalid graph file: " + graphFileNameAbsolutePath);
            return;
        }
