...
import scaffold.presentation.uml.UMLNode.UMLNodeType;
import scaffold.modeling.uml.ClassModel;
import scaffold.modeling.uml.basic.UmlPackage;
import scaffold.modeling.uml.basic.cm.CmClass;
import scaffold.modeling.uml.basic.cm.CmAssociation;
import scaffold.platform.graphics.graph.GraphCanvas;
import scaffold.platform.graphics.graph.Node;
import scaffold.platform.graphics.graph.Link;
import scaffold.platform.graphics.draggable.IDraggable;
import scaffold.platform.ErrorDialog;
import scaffold.platform.fileexplorer.FileUtility;
...
public class UMLCanvas extends GraphCanvas {  
    ...
    public UMLLink addUMLInheritanceLink(UMLEndPoint e1, UMLEndPoint e2, int ep1Location, int ep2Location, double percent) {
    ...
    public UMLLink addUMLAssocOrAggregationLink(boolean isAggregation,
            UMLEndPoint ep1, UMLEndPoint ep2, int ep1Location, int ep2Location, final double percent) {
        ...
        if (ep1Location == ep2Location) {
            if (1.0 - percent - 0.2 > 0.0) {
                percent2 = percent + 0.2;
            } else {
                if (percent - 0.2 >= 0.0) {
                    percent2 = percent - 0.2;
                } else {
        ...
        ep1.setLocationByPercent(ep1Location, percent);
    ...
    protected void doCreateNode(String[] nodeElements) {
        if (nodeElements.length < 4) {
            ErrorDialog.error("Invalid graph file: expecting 4 fields in the node descriptor");
            return;
        }
    ...
    protected void doCreateLink(String[] linkElements) {
        ...
        if (linkElements.length < 10) {
            ErrorDialog.error("Invalid graph file: expecting 10 fields in the link descriptor");
            return;
        }
    ...
    private UMLLink addUMLLink(
        ...
        UMLEndPoint ep1 = new UMLEndPoint(ept1, clazz1, this);
        UMLEndPoint ep2 = new UMLEndPoint(ept2, clazz2, this);
