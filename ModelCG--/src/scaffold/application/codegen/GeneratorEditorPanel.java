...
public class GeneratorEditorPanel extends JPanel {
    ...
    public void collectPropertyValue(String propertyName) {
        ...
        if (action == null) {
            test.platform.ErrorDialog.error("Why is there a description when there is no action selected?");
            return;
        }
        ...
        if (markerIndex != -1) { //this is an Expression Property
            ...
            if (property == null) {
                test.platform.ErrorDialog.error("Can't find property: " + propertyName);
                return;
            }
